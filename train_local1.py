...
from loss_local1 import FocalLoss
...
net=torch.load('./checkpoint/adam_e4_iou45_pre_fpn50_full_b2_dota_50_nd_9ma_s20_105_5block1.pkl')
fpn=net.module.fpn
fpn.eval()
net=net.module
if resume:
...
def train(epoch):
    ...
    for batch_idx, (inputs, loc_targets, cls_targets, flip) in enumerate(t):
        ...
        loss,loc_loss,cls_loss,iw= criterion(inputs, fpn,loc_preds, loc_targets, cls_preds, cls_targets)
	...
	#print(iw)
        ...
        t.set_description('loc_loss: %.3f|cls_loss: %.3f|iw: %.2f | train_loss: %.3f | avg_loss: %.3f, %d' % (loc_loss, cls_loss, iw.cpu().numpy()[0], loss.data[0], train_loss/(batch_idx+1),flip[0]))
...
#t.set_description('loc_loss: %.3f|cls_loss: %.3f|iw: %.2f | train_loss: %.3f | avg_loss: %.3f, %d' % (loc_loss, cls_loss, iw, loss.data[0], train_loss/(batch_idx+1),flip[0]))
	...
	test_loss += loss.data[0]
        ...
        t.set_description('loc_loss: %.3f|cls_loss: %.3f|iw: %.2f | test_loss: %.3f | avg_loss: %.3f, %d' % (loc_loss, cls_loss, iw.cpu().numpy()[0], loss.data[0], test_loss/(batch_idx+1),flip[0]))
	#print('test_loss: %.3f | avg_loss: %.3f' % (loss.data[0], test_loss/(batch_idx-count+1)))
    # Save checkpoint
    global best_loss
    ...
    if test_loss < best_loss:
        ...
        torch.save(net, './checkpoint/adam_e4_iou45_pre_fpn50_full_b2_50_nd_9ma_s20_105_5ges_8l.pkl' )
        best_loss = test_loss
...
for epoch in range(start_epoch, start_epoch+50):
    ...
    with open('/group/proteindl/ps793/Dota/retina/train_loss_e4_full_adam_nd_9ma_iou45_b2_fpn50_s20_105_5ges_8l.csv', 'wb') as myfile:
    	wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    	wr.writerow(train_loss_list)

    with open('/group/proteindl/ps793/Dota/retina/test_loss_e4_full_adam_nd_9ma_iou45_b2_fpn50_s20_105_5ges_8l.csv', 'wb') as myfile:
    	wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    	wr.writerow(test_loss_list)
    if epoch % 5 ==0:
       torch.save(net, './checkpoint/adam_e4_iou45_pre_fpn50_full_b2_dota_50_nd_9ma_s20_105_5ges_8l_epoch_%d.pkl' % epoch )

torch.save(net, './checkpoint/adam_e4_iou45_pre_fpn50_full_b2_dota_50_nd_9ma_s20_105_5ges_8l_final.pkl' )   
